        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 以前的路由跳转：从A路由跳转到B路由，这里再加入购物车，进行路由跳转之前，发请求把购买的产品信息
                     通过请求的形式发送给服务器，服务器进行相应的存储 
                -->
...
export default {
  ...
  methods: {
    // 排他思想，让所有 isChecked 都为 0 ，当前点击的为 1
    ...
    // 用户点击输入框来改变产品个数
    changeCount(e) {
      // 用户输入的文本 * 1  如果是 数字 * 1 则还是数字 但如果是其他的 则是 NaN
      let value = e.target.value * 1
      // 判断是否合法
      if(isNaN(value) || value < 1) {
        this.count = 1
      }else{
        // 合法情况【但要考虑是否会为 小数 ，所以直接取整 】
    ...
    async addShopCart(){
      ...
      // 3.失败：给用户提示
      /*
      this.$store.dispatch('detail/addShopCart',{skuId,skuNum}) 
      这行代码其实就是调用仓库里的addOrUpdateShopCart方法
      这个方法再加上 async 则返回的一定是一个 Promise 对象  Promise对象只要返回值不是空对象，那就都为成功
      */
      
      try {
        // 成功
        await this.$store.dispatch('detail/addOrUpdateShopCart',{skuId,skuNum})
        // 进行路由跳转 并且将产品信息带给 addcartsuccess 路由组件
        // 因为要传的参数太多，所以使用 会话存储帮着存储数据，然后去那个页面直接拿
        this.$router.push({
          name:'addcartsuccess',
          query:{
            skuNum:this.count
